...
class UnitPNFileFixtures extends Fixture implements FixtureGroupInterface
{
    ...
    public function __construct(private readonly string $unitsPNFixturesPath)
    {
    }
    ...
    {
        if (file_exists($this->unitsPNFixturesPath)) {
            if ($manager instanceof EntityManagerInterface) {
                $manager->getConnection()->getConfiguration()->setSQLLogger(null);
            }
            $row = 1;
            $handle = fopen($this->unitsPNFixturesPath, 'rb');
