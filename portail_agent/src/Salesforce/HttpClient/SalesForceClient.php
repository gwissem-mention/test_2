    ...
    public function sendEvent(SalesForceApiEventDefinition $eventDefinition): void
    {
        try {
            ...
            if (Response::HTTP_CREATED !== $response->getStatusCode()) {
                throw new SendEventException(sprintf('Failing sending event, status code %s, response content %s', $response->getStatusCode(), $response->getContent(false)));
            }

            $this->salesforceLogger->info(sprintf('Successfully event sent, response content %s', $response->getContent()));
